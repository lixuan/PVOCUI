...
export function getVal(val) {
  if (toString.call(val) === '[object Number]') { return val } else {
    return val || ''
  }
}
...
export function importFile(fileArr, func) {
  ...
  const fileData = fileArr.map(item => {
    return {
      name: item.name,
      file: item,
  ...
  const parseXlsx = (file) => {
    ...
    reader.onload = (e) => {
      ...
      if (fileData.filter(item => !item.parseData).length === 0) { func(fileData) }
    }
  ...
  fileData.forEach(item => {
    if (/xlsx|xls|XLS|XLSX/.test(item.extensionName)) { parseXlsx(item.file) } else {
      parseText(item.file)
    }
  })
}
...
export function exportFile({ title, titleStartRow = 0, titleStartCol = 0, data, header, fileName, sheetName = 'sheet1', startRow = 1, startCol = 0, dataStyle, headerStyle, Func }) {
  ...
  // 处理data和header数据
  const titleArr = Object.values(header)
  const keyArr = Object.keys(header)
  data = data.map(item => {
...
export function getColFlag(colIndex) {
  ...
  if (floorVal === 0) { return letterArr[colIndex - 1] } else {
...
export function mergeCell(ws, startCell, endCell) {
  ...
  const startCol = getColIndex(startCell.match(/^[A-Z]+/)[0])
  const startRow = parseInt(startCell.match(/^[A-Z]+(\d+)$/)[1]) - 1
  const endCol = getColIndex(endCell.match(/^[A-Z]+/)[0])
  const endRow = parseInt(endCell.match(/^[A-Z]+(\d+)$/)[1]) - 1
...
function setXlsxRange(ws) {
  ...
  const rowArr = [...new Set(keyArr.map(item => {
    const regArr = item.match(/^[A-Z]+/)
    if (regArr && toString.call(regArr) === '[object Array]' && regArr.length > 0) { return getColIndex(regArr[0]) }
    // return regArr[0].match(/[A-Z]{1}/g).reduce((total,currentValue,currentIndex) => {
    //   return total + letterArr.findIndex(ele => ele === currentValue) + (currentIndex ? 1 : 0)
    // }, 0)
    else { return undefined }
  }))].filter(item => item != undefined).sort(numSortFunc)
  const colArr = [...new Set(keyArr.map(item => {
    const regArr = item.match(/^[A-Z]+(\d+)$/)
    if (regArr && toString.call(regArr) === '[object Array]' && regArr.length > 1) { return (parseInt(regArr[1]) - 1) } else { return undefined }
  }))].filter(item => item != undefined).sort(numSortFunc)
...
function siteColWidth(data) {
  ...
  const colWidth = data.map(row => row.map(val => {
    ...
    if (!val) {
      ...
      const chineseArr = val.toString().match(/[\u4e00-\u9fa5]/g)
      const noChineseArr = val.toString().match(/[^(\u4e00-\u9fa5)]/g)
      const len = ((chineseArr && chineseArr.length) || 0) * 2 + ((noChineseArr && noChineseArr.length) || 0)
...
export function customColWidth({ ws, colFlag, type = 'wch', width }) {
  ...
  // 列索引
  const regArr = colFlag.match(/^[A-Z]+/)
  let colIndex = 0
  ...
  if (type === 'wch') {
    const val = ws[colFlag] && ws[colFlag]['v'].toString()
    if (val) {
      const chineseArr = val.toString().match(/[\u4e00-\u9fa5]/g)
      const noChineseArr = val.toString().match(/[^(\u4e00-\u9fa5)]/g)
      const len = ((chineseArr && chineseArr.length) || 0) * 2 + ((noChineseArr && noChineseArr.length) || 0)
      if (ws['!cols'][colIndex] && ws['!cols'][colIndex].wch && ws['!cols'][colIndex].wch < len) { ws['!cols'][colIndex] = { wch: len } }
      if (!ws['!cols'][colIndex]) { ws['!cols'][colIndex] = { wch: len } }
    } else { ws['!cols'][colIndex] = { wch: 10 } }
  } else {
...
function dataTable_to_sheetFormat(title, titleStartRow, titleStartCol, data, startRow, startCol, headerStyle, dataStyle) {
  ...
  const getStyle = rowNum => {
    if (rowNum === 0 && headerStyle) { return deepClone(headerStyle) }
    if (rowNum === 0 && !headerStyle) { return deepClone(headerCellStyle) }
    if (rowNum !== 0 && dataStyle) { return deepClone(dataStyle) }
    if (rowNum !== 0 && !dataStyle) { return deepClone(dataCellStyle) }
    return null
  }
  ...
  for (let R = 0; R < data.length; R++) {
    for (let C = 0; C < data[R].length; C++) {
      ...
      if (toString.call(cell.v) === '[object Number]') { cell.t = 'n' } else if (toString.call(cell.v) === '[object Boolean]') { cell.t = 'b' } else if (toString.call(cell.v) === '[object Date]') {
        cell.t = 'd'
      } else { cell.t = 's' }
...
Data.prototype = {
  key: function(page) {
    const descriptor = {}; let unlock = page[this.expando]
    if (!unlock) {
      unlock = Data.uid++
