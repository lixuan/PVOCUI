...
service.interceptors.request.use(config => {
  ...
  // console.log(config.url.substring(0,4))
  if (process.env.NODE_ENV === 'production' && config.url.substring(0, 4) == '/api') {
    config.url = 'http://liyiming.weixin188.top/DBService' + config.url
  } else {
    config.url = 'http://liyiming.weixin188.top' + config.url
  }
