<template>
  <div
    id="equTree"
    v-loading="loading"
    class="templateTree playBill-equ-tree-loading"
    element-loading-text="加载中"
    element-loading-background="rgba(255, 255, 255, 0.7)"
    :class="{ hidd: showStyle }"
  >
    <el-scrollbar class="page-component__scroll">
      <div class="templateTree-search">
        <el-input v-model="searchTreeNode" size="mini" placeholder="请输入关键字" @input="onSearchNode">
          <i slot="suffix" class="el-input__icon el-icon-search" style="cursor: pointer;" @click="onSearchNode" />
        </el-input>
      </div>
      <div class="templateTree-tree">
        <el-tree
          ref="tree"
          :data="pTreeData"
          :show-checkbox="pShowCheckbox"
          :node-key="pTreeDefaultProps.uuid"
...
export default {
  ...
  methods: {
    ...
    filterNode(value, data) {
      ...
      if (data[this.pTreeDefaultProps.label] == undefined) { return false }
    ...
    setParentDisplayed(searchedNode) {
      const _this = this
      const aaa = []
      if (searchedNode.length == 0) { return }
      // if(searchedNode.includes('-1')||searchedNode.includes('0'))
      //     return;
      if (searchedNode.includes('-1')) { searchedNode.splice(searchedNode.indexOf('-1'), 1) }
      if (searchedNode.includes('0')) { searchedNode.splice(searchedNode.indexOf('0'), 1) }
    ...
    getChildrenNode1(node, searchedNode) {
      const _this = this
      if (searchedNode.includes(node[_this.pTreeDefaultProps.uuid])) { node.disabled = false }
      node.children.forEach((item, index) => {
        if (item.children == undefined) // 叶子节点
        {
          if (searchedNode.indexOf(item[_this.pTreeDefaultProps.uuid].toString()) > -1) { item.disabled = false }
        } else {
    ...
    disableNodeById(nodeId) {
      for (const item of this.pTreeData) {
    ...
    getChildrenNode(node) {
      if (this.pChoosedNode.indexOf(node[this.pTreeDefaultProps.id].toString()) > -1) { this.choosedNodeIDs.push(node[this.pTreeDefaultProps.uuid].toString()) }
      node.children.forEach((item, index) => {
        ...
        // }
        if (this.pChoosedNode.indexOf(item[this.pTreeDefaultProps.id].toString()) > -1) { this.choosedNodeIDs.push(item[this.pTreeDefaultProps.uuid]) }
        if (item.children && item.children.length > 0) { this.getChildrenNode(item) }
      })
    },
    // 区域外点击事件
    handleDocumentClick(e) {
      if (!this.$el.contains(e.target)) {
        const el = this.$parent.$children.filter(item => item.$el.className === 'chsEqp')
        if (el.length == 0) { return }
        if (el[0].$el.contains(e.target)) { return }
