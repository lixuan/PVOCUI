...
export function sortByDate(data) {
  return data.sort(function(a, b) {
    const Bdate = parseTime(b.createDate)
    const Adate = parseTime(a.createDate)
...
export function parseDic(val) {
  if (val) {
    const cachestr = window.localStorage.getItem('DicCaches')
    let caches = null
    if (cachestr && cachestr.length > 0) {
      caches = JSON.parse(cachestr)
      const cacheValue = caches.find((item, index) => { if (item.key == val) { return item } })
      if (cacheValue != null && cacheValue != undefined && cacheValue.create) {
        const time = new Date().getTime() - Number(cacheValue.create)
