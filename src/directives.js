...
// v-drawerDrayWidth: 弹窗宽度拖大 拖小
Vue.directive('drawerDrayWidth', {
  bind(el, binding, vnode, oldVnode) {
    const dragDom = el.querySelector('.el-dialog__wrapper')
    // const dragDom = binding.value.$el.querySelector('.el-dialog__wrapper');

    el.onmousedown = (e) => {
      // 鼠标按下，计算当前元素距离可视区的距离
      const disX = e.clientY - el.offsetTop

      document.onmousemove = function(e) {
        e.preventDefault() // 移动时禁用默认事件

        // 通过事件委托，计算移动的距离
        const l = e.clientY - disX
