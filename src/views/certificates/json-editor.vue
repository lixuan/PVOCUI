<template>
  <div class="app-container">
    ...
    />
    <el-dialog title="证书邮寄信息" :visible.sync="dialogDetailsVisible">
      <el-form :label-position="labelPosition" label-width="120px" :model="formLabelAlign">
        <el-row>
          <el-col :xs="24" :sm="24" :lg="12">
            <el-form-item label="申请单号：">
              {{ this.formLabelAlign.busNO }}
            </el-form-item>
            <el-form-item label="快递公司：">
              <el-input v-model="formLabelAlign.deliveryCompany" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :lg="12">
...
import {
  GetBusinessList, getCreateOrUpdatePort, getPostInfoForEdit, getPostInfo
} from '@/api/business'
...
export default {
  ...
  methods: {
    ...
    handlePublish(row) {
      ...
      this.formLabelAlign.busNO = row.busNO
    },
    // 添加证书邮寄信息
    assignmentsDataClick() {
      const data = {
        busNO: this.formLabelAlign.busNO,
        deliveryCompany: this.formLabelAlign.deliveryCompany,
        deliveryNO: this.formLabelAlign.deliveryNO
      }
      GetBusinessList(data).then(res => {
        if (res.success) {
          this.dialogDetailsVisible = false
          this.$message.success('证书邮寄信息添加成功')
        }
      })
    }
  }
